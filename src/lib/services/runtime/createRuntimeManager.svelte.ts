...
import { buildWorkspaceTree } from '$lib/utils/vfs.js';
...
function getRootWorkspaceTree(projects: RepoLayoutData['projects']) {
	return buildWorkspaceTree(projects);
}
...
export function createRuntimeManager(options: Options) {
	...
	async function startRuntime() {
		...
		try {
			...
			const workspaceTree =
				options.getWorkspaceTree?.() ?? getRootWorkspaceTree(options.getProjects());
			await wc.mount(workspaceTree);
