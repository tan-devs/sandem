/**
 * ExplorerController.svelte.ts
 *
 * Controls Explorer tree selection state and maps file/directory clicks to
 * workspace actions (open file, select project, create/rename/delete).
 *
 * Previously named PanelController.svelte.ts — renamed because this has
 * nothing to do with panel visibility. That belongs to PanelsController.
 */
...
type CreateWorkspaceTreeControllerOptions = {
...
export function createWorkspaceTreeController(options: CreateWorkspaceTreeControllerOptions) {
...
export type WorkspaceTreeController = ReturnType<typeof createWorkspaceTreeController>;
